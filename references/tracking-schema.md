...
- `render.deadlift_variation` and `render.deadlift_execution`: required for deadlift AI score V1. Only exact `conventional` plus `reset_single` is eligible; the score engine refuses absent, pause, deficit, sumo, touch-and-go, or otherwise unverified variants. The source clips must be one reset repetition plus bound side/oblique-side and rear views, with source-matched pose companions and strict available side `bar_tracking`.
- `deadlift-score.json`: an optional derived audit file emitted by `scripts/deadlift_scoring.py`. It has `{schema_version, scorable, total, grade, items, unavailable_reason}`. Each item stores `id`, `max_score`, `score`, `status`, `evidence_frames`, `metrics`, and the 2D `limitation`; these are internal traceability fields. The public renderer exposes only total and a `SS/S/A/B/C/D` badge, exclusively on Page 4. If `scorable` is false, Page 4 shows no numeric score, grade, or training prescription.
