...
```json
{
  ...
  "render": {
    ...
    "deadlift_variation": "conventional",
    "deadlift_execution": "reset_single",
    "anatomy_indices": [{
      "number": "①",
      "muscle": "股四头肌群",
      "view": "正面",
...
- `render.deadlift_variation` and `render.deadlift_execution`: optional classification fields for deadlift AI score V1. An explicitly identified pause, deficit, sumo, or touch-and-go variant is ineligible; absent fields do not block ordinary-video scoring. A usable side/oblique side bar path plus bound front/rear endpoint evidence remains required.
- `deadlift-score.json`: an optional derived audit file emitted by `scripts/deadlift_scoring.py`. It has `{schema_version, scorable, total, grade, items, unavailable_reason}`. Each item stores `id`, `max_score`, `score`, `status`, `source`, `evidence_frames`, `metrics`, and the 2D `limitation`. A hidden or low-confidence individual pose landmark becomes `source: "中性基准"` and receives 80% of that rule's weight; it is not an observed fault and cannot create a muscle direction or training drill. The public renderer exposes only total and a `SS/S/A/B/C/D` badge, exclusively on Page 4, with one concise neutral-baseline note where needed. Its score settlement uses equal left/right columns: a centred large `总分＋分` group on the left, and a centred grade seal plus its caption on the right. If core camera/bar evidence is unavailable, Page 4 shows no numeric score, grade, or training prescription.
- `squat-score.json`: an optional derived audit file emitted by `scripts/squat_scoring.py`. It has the same score fields plus `phase_detection: {available, phases, phase_records, pause_bottom, representative_frames}`. Phase records store source frames/times, confidence, and the signals used. The detector is side-view only; a rear recording supplies only bilateral bar-level evidence and is never time-aligned to the side repetition. `SQ-01..SQ-06` use weights `20/15/20/20/15/10`; hidden pose-only items receive the same 80% neutral baseline. A detected pause, box, or tempo variant is retained but marked unscorable for conventional-squat scoring.
